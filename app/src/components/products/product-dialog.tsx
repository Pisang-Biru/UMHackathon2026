...
export function ProductDialog({ open, onOpenChange, initial, onSubmit }: ProductDialogProps) {
  ...
  const [cogs, setCogs] = React.useState(initial?.cogs != null ? initial.cogs.toString() : '')
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setCogs(initial?.cogs != null ? initial.cogs.toString() : '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const parsedCogs = cogs.trim() === '' ? null : parseFloat(cogs)
    ...
    if (parsedCogs !== null && (isNaN(parsedCogs) || parsedCogs < 0)) return setError('COGS must be a valid non-negative number')
    ...
    setError(null)
    try {
      await onSubmit({ name: name.trim(), price: parsedPrice, stock: parsedStock, description, cogs: parsedCogs, packagingCost: parsedPackagingCost })
      onOpenChange(false)
    } catch (err) {
